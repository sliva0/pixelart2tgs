...
class Chain:
    def __init__(self, first_frame_index: int, first_visible_frame: FormPosColor):
        self.frames: ChainFramesType = [None] * first_frame_index + [first_visible_frame]
...
def extract_frame_shapes(frame: np.ndarray) -> FrameShapesType:
    x, y, z = frame.shape
    colors = get_colors(frame.reshape(x * y, z))
...
def extract_animation_shapes(source_animation: SourceAnimationType) -> tuple[tuple[float], AnimationShapesType]:
    shape_dict = defaultdict(list)

    durations, frames = zip(*source_animation)
